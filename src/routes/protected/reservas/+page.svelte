		...
		<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
			{#each data.reservas as r}
				...
				>
					...
					{#if r.tour?.imagen}
						<img
							src={`${PUBLIC_HOST}/public${r.tour.imagen}`}
							alt={r.tour.nombre}
					...
					<div class="flex items-start justify-between gap-3">
						...
						<!-- Estado (este endpoint no lo devuelve, así que lo dejamos fijo) -->
						<span
					...
					<div class="mt-3 text-sm text-slate-700 dark:text-white/70">
						<div>
							<span class="font-semibold">Personas:</span>
							{r.numero_personas}
						</div>

						<div>
							<span class="font-semibold">Horario:</span>
							{r.tour?.hora_inicio} – {r.tour?.hora_fin}
						</div>

						<div>
							<span class="font-semibold">Total:</span> ${r.total}
						</div>
					</div>
				</div>
			{/each}
