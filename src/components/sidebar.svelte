...
<div class="relative">
		...
		{#if user.rol_id != 1}
			<SidebarGroup>
				{#each userSidebar as { label, href }}
					<SidebarItem {label} {href} {spanClass}></SidebarItem>
				{/each}
			</SidebarGroup>
		{:else}
			<SidebarDropdownWrapper label="CRUD" classes={{ btn: 'p-2' }}>
				<SidebarItem label="Usuarios" href="/admin/users" />
				<SidebarItem label="Tours" href="/admin/tours" />
			</SidebarDropdownWrapper>
		{/if}
		<SidebarGroup border>
	...
	<div class="grid overflow-auto px-4 md:ml-64">
		<div class="min-h-screen max-w-full rounded-lg border-gray-200 p-4 dark:border-gray-700">
			{@render children()}
		</div>
	</div>
</div>
